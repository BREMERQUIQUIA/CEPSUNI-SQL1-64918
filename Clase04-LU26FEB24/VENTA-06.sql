
-- Creacion de la tabla venta.
-- Con PK y FK.
-- Validar columnas

DROP TABLE ABC..VENTA;
GO

CREATE TABLE ABC..VENTA(
	IDVENTA INT NOT NULL IDENTITY(1,1),
	IDCLIENTE INT NULL,
	PRODUCTO VARCHAR(100) NOT NULL DEFAULT 'PRODUCTO GANCHO',
	PRECIO NUMERIC(8,2) NOT NULL,
	CANTIDAD INT NOT NULL,
	IMPORTE NUMERIC(8,2) NOT NULL,
	CONSTRAINT PK_VENTA PRIMARY KEY(IDVENTA),
	CONSTRAINT FK_VENTA_CLIENTE 
		FOREIGN KEY(IDCLIENTE) REFERENCES ABC..CLIENTE,
	CONSTRAINT CK_VENTA_PRECIO CHECK(PRECIO>0),
	CONSTRAINT CK_VENTA_CANTIDAD CHECK(CANTIDAD>0),
	CONSTRAINT CK_VENTA_IMPORTE CHECK(PRECIO*CANTIDAD=IMPORTE)
);
GO

SELECT * FROM ABC..CLIENTE;
GO

-- Esta instruccion se ejecuta correctamente.
INSERT INTO ABC..VENTA(IDCLIENTE,PRECIO,CANTIDAD,IMPORTE)
VALUES(100,4.0,5,20);
GO

INSERT INTO ABC..VENTA(IDCLIENTE,PRECIO,CANTIDAD,IMPORTE)
VALUES(NULL,4.0,5,20);
GO

INSERT INTO ABC..VENTA(PRODUCTO,PRECIO,CANTIDAD,IMPORTE)
VALUES('LAPTOP',4.0,5,20);
GO

SELECT * FROM ABC..VENTA;
GO



